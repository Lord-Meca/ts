...
SWEP.WorldModel = "models/naruto/unique/unique2/foc_nr_unique2_bane.mdl"
...
if SERVER then
    function ApplyKnockback(npc, force)
        if IsValid(npc) and npc:IsNPC() then
            -- Obtenir la direction vers l'arrière par rapport à l'orientation actuelle du NPC
            local direction = -npc:GetForward()  -- Pousse dans la direction opposée à celle où le NPC fait face
            
            -- Appliquer la force
            local physObj = npc:GetPhysicsObject()
            if IsValid(physObj) then
                physObj:SetVelocity(direction * force)  -- Appliquer la force
            end
        end
    end
...
function SWEP:Reload()
	...
	local particleName = "vj_explosionfire1"
	local attachment = ply:LookupBone("ValveBiped.Bip01_R_Hand")


    timer.Simple(0.7, function()

		self:DoCombo( AttackHit1, 11, 300, 0, 0.16, "weapon_art", Angle(3, -3, 0),0, 0, Combo1, 0.14, false, false, 0, 0,false,true)
