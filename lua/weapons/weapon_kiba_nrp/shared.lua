...
SWEP.WorldModel = "models/naruto/unique/unique7/foc_nr_unique7_bane.mdl"
...
function SWEP:Reload()
    ...
    timer.Simple(0.3, function()
		...
		local tr = util.TraceLine({
			start = ply:GetPos() + Vector(0, 0, 50), 
			endpos = ply:GetPos() + ply:GetAimVector() * distance,
			filter = ply
		})

		local impactPos = tr.HitPos

		ParticleEffect("[0]_white_thunderbolt", impactPos, Angle(0, 0, 0), nil)

		local entities = ents.FindInSphere(impactPos, 300)
		for _, entity in pairs(entities) do
			if IsValid(entity) and (entity:IsPlayer() or entity:IsNPC()) and entity ~= ply then
				local damageInfo = DamageInfo()
				damageInfo:SetDamage(300)
