if (SERVER) then
	...
	util.AddNetworkString("Sword_State")
end
...
SWEP.WorldModel = "models/naruto/epee/epee14/foc_nr_epee14_bane.mdl"
...
function SWEP:Deploy()
	...
	if SERVER then
		net.Start("Sword_State")
		net.WriteEntity(self.Owner)
		net.WriteBool(self.swordHolstered)
		net.Broadcast()
	end

	self.Owner:SetModel("models/falko_naruto_foc/body_upper/man_overall_01.mdl")
...
hook.Add("PostPlayerDraw", "SwordModelInBack", function(ply)
    ...
    if not IsValid(activeWeapon) or activeWeapon:GetClass() ~= "weapon_bukijutsu" then return end

    if activeWeapon.swordHolstered then
        if not IsValid(ply.swordModelHolster) then
            local model = ClientsideModel("models/naruto/epee/epee14/fsc_nr_epee14_bane.mdl")
        ...
        pos = pos + ang:Forward() * 35 + ang:Up() * 25 + ang:Right() * -15
...
function SWEP:Reload()
	...
	ply:SetAnimation(PLAYER_RELOAD)
	-- ParticleEffectAttach("nrp_eden_kitsushi", PATTACH_POINT_FOLLOW, self, 7)
...
hook.Add("PlayerButtonDown", "bukijutsuSweps", function(ply, button)

	local activeWeapon = ply:GetActiveWeapon()

    if not IsValid(activeWeapon) or activeWeapon:GetClass() ~= "weapon_bukijutsu" then
        return
    end
...
if CLIENT then
    net.Receive("Sword_State", function()
        ...
        if IsValid(activeWeapon) and activeWeapon:GetClass() == "weapon_bukijutsu" then
