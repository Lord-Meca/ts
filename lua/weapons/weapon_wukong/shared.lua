...
if (CLIENT) then
	...
	SWEP.SlotPos = 1
	SWEP.DrawAmmo = false
	SWEP.PrintName = "Bâton du Roi des Singes"
...
function SWEP:Think()
	...
	--print(self.postureBar)

    if ply:KeyDown(IN_RELOAD) then
        if IsValid(self) and ply:IsOnGround() then
...
function SWEP:Reload()
	...
	-- local particleName = "[4]_red_aura"
	-- local attachment = ply:LookupBone("ValveBiped.Bip01_R_Foot")
	...
	-- local particleName = "[1]_raiton_hit_aura"
	-- local attachment = ply:LookupBone("ValveBiped.Bip01_R_Foot")
	...
	-- 	local particleName1 = "izoxfoc_taijutsu_bonification_violet_bis_b"
	-- 	local attachment = ply:LookupBone("ValveBiped.Bip01_R_Foot")
...
function SWEP:StartTransformation()
    local ply = self.Owner

    if not IsValid(ply) then return end

    local particleName1 = "izoxfoc_taijutsu_bonification_violet_bis_a"
    local attachment = ply:LookupBone("ValveBiped.Bip01_R_Foot")
    ...
    timer.Simple(0.5, function()
        if IsValid(ply) then
            --ply:SetModel("models/player/dasheng.mdl")

            ply:StopParticles()

            -- local particleName2 = "izoxfoc_taijutsu_bonification_violet_bis_d"
            -- local attachment = ply:LookupBone("ValveBiped.Bip01_R_Foot")

            -- if attachment then
            --     ParticleEffectAttach(particleName2, PATTACH_ABSORIGIN_FOLLOW, ply, attachment)
            -- end
