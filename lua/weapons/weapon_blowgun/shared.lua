...
SWEP.WorldModel = "models/weapons/w_foc_blowgun.mdl"
...
function shootBlowgun(ply, self)
    ...
    hook.Add("Think", "ballMove" .. ent:EntIndex(), function()
        ...
        if trace.Hit then
            ...
            ent:Remove()
            hook.Remove("Think", "ballMove" .. ent:EntIndex())
        else
...
function SWEP:Reload()
    ...
    self.NextSpecialMove = CurTime() + 1.5

    self:SetHoldType("weapon_art")
    ply:SetAnimation(PLAYER_RELOAD)

    timer.Simple(0.2, function()
		shootBlowgun(ply,self)
    end)
end
