if (SERVER) then
	...
	util.AddNetworkString("Nuibari_State")
end
...
function SWEP:Deploy()
	...
	if SERVER then
		net.Start("Nuibari_State")
		net.WriteEntity(self.Owner)
		net.WriteBool(self.nuibariHolstered)
		net.Broadcast()
	end

	self.Owner:SetModel("models/falko_naruto_foc/body_upper/man_coat_04.mdl")
...
hook.Add("PostPlayerDraw", "NuibariModelInBack", function(ply)
    local activeWeapon = ply:GetActiveWeapon()
    if not IsValid(activeWeapon) or activeWeapon:GetClass() ~= "weapon_nuibari" then return end

    if activeWeapon.nuibariHolstered then
        if not IsValid(ply.nuibariModelHolstered) then
            local model = ClientsideModel("models/naruto/unique/unique5/foc_nr_unique5_bane.mdl")
            if IsValid(model) then
                model:SetNoDraw(true)
                ply.nuibariModelHolstered = model
            end
        end
        ...
        pos = pos + ang:Forward() * 40 + ang:Up() * 25 + ang:Right() * -20
        ang:RotateAroundAxis(ang:Forward(), 180)
        ang:RotateAroundAxis(ang:Right(), 30)
        ang:RotateAroundAxis(ang:Up(), 180)

        if IsValid(ply.nuibariModelHolstered) then
            ply.nuibariModelHolstered:SetRenderOrigin(pos)
            ply.nuibariModelHolstered:SetRenderAngles(ang)
            ply.nuibariModelHolstered:DrawModel()
        end
    else
        if IsValid(ply.nuibariModelHolstered) then
            ply.nuibariModelHolstered:Remove()
        end
    end
end)

local function grapPlayers(ply,self)

	if not SERVER then return end

	local playerAttachPos = ply:EyePos() - Vector(0, 0, 20)

	for _, entity in pairs(ents.FindInSphere(ply:GetPos(), 500)) do
...
hook.Add("PlayerButtonDown", "nuibariSweps", function(ply, button)

	local activeWeapon = ply:GetActiveWeapon()

    if not IsValid(activeWeapon) or activeWeapon:GetClass() ~= "weapon_nuibari" then
        return
    end
    ...
    if button == MOUSE_MIDDLE then
        ...
        activeWeapon.nuibariHolstered = not activeWeapon.nuibariHolstered

        if SERVER then
            net.Start("Nuibari_State")
            net.WriteEntity(ply)
            net.WriteBool(activeWeapon.nuibariHolstered)
            net.Broadcast()
        end
...
if CLIENT then
    net.Receive("Nuibari_State", function()
        ...
        if IsValid(activeWeapon) and activeWeapon:GetClass() == "weapon_nuibari" then
            activeWeapon.nuibariHolstered = holstered
